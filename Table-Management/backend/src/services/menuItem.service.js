...
export class ItemService{
    ...
    static async update(id, data) {
        ...
        if ((data.name && data.name !== item.name) || (data.category_id && data.category_id !== item.category_id)) {
    ...
    static validateItemData(data, isUpdate = false) {
        ...
        if (data.prep_time_minutes !== undefined) {
            if (typeof data.prep_time_minutes !== 'number' || !Number.isInteger(data.prep_time_minutes)) {
        ...
        if (data.status !== undefined) {
            if (!['available', 'unavailable', 'sold_out'].includes(data.status)) {
                errors.push('Status must be either "available", "unavailable", or "sold_out"');
            }
        }
